...
/*
	...
	说明:
		1、
*/
	CLog::Log(LOGDEBUG,"==========>>> cyk CFile::Open   file :: %s", strFileName.c_str()); /* 打印传入的文件名字*/
	...
	try
	{
		...
		CLog::Log(LOGDEBUG,"==========>>> cyk   CFile::Open   protocol::%s  url::%s", strProtocol.c_str(), url.Get().c_str());
		#endif

		m_pFile = CFileFactory::CreateLoader(url);/* 见函数内部根据不同的协议类型创建不同的具体实例*/
		if (!m_pFile)
			return false;
...
*/
	...
	if(m_pBuffer)
	{
		...
		{
			unsigned int nBytes = m_pBuffer->sgetn((char *)lpBuf, min<streamsize>((streamsize)uiBufSize,m_pBuffer->in_avail()));
...
/*
	...
	说明:
		1、
*/
	//CLog::Log(LOGDEBUG,"==========>>> cyk CFile::Seek   iFilePosition=%d  iWhence=%d", iFilePosition, iWhence); 
...
CFileStreamBuffer::pos_type CFileStreamBuffer::seekpos(pos_type pos, ios_base::openmode mode)
{
/*
	参数:
		1、
