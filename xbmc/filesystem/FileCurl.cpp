...
*/
	...
	{
		if( !h.GetValue("icy-notice1").IsEmpty()|| !h.GetValue("icy-name").IsEmpty()|| !h.GetValue("icy-br").IsEmpty() )
			h.Parse("Content-Type: audio/mpeg\r\n");
	}

	/* hack for google video */
	if ( h.GetMimeType().Equals("text/html")&&  !h.GetValue("Content-Disposition").IsEmpty() )
	{
...
*/
	...
	if( strProtocol.Equals("ftp")||  strProtocol.Equals("ftps") )
	{
	...
	else if( strProtocol.Equals("http") ||  strProtocol.Equals("https"))
	{
