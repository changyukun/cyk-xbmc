...
/*
	...
	说明:
		1、此构造函数内实例了一个CCacheStrategy 缓冲cache 类
*/
	m_bDeleteCache = true;
	m_nSeekResult = 0;
	m_seekPos = 0;
	...
	{
		/*创建一个循环buffer */
		m_pCache = new CCacheCircular(g_advancedSettings.m_cacheMemBufferSize , std::max<unsigned int>(g_advancedSettings.m_cacheMemBufferSize /4, 1024 * 1024));
	}
...
*/
	...
	/* ----------> 打开cache */
	// open cache strategy
	if (m_pCache->Open() != CACHE_RC_OK) /* 此open 方法调用的是哪个，要看m_pCache 指向的实例是哪个类的，见构造函数CFileCache::CFileCache() 中new 的实例*/
	{
	...
	if (!m_source.Open(m_sourcePath, READ_NO_CACHE | READ_TRUNCATED | READ_CHUNKED)) /* 此处又调用了CFile::Open() 方法进行打开实际的文件*/
	{
