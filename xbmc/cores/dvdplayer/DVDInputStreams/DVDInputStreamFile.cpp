...
*/
	...
	CLog::Log(LOGDEBUG,"==========>>> cyk CDVDInputStreamFile::Open() file=%s",strFile);

	if (!CDVDInputStream::Open(strFile, content))
		return false;

	m_pFile = new CFile();/* 实例一个CFile 类，即"  CFile 实例" */
	if (!m_pFile)
		return false;
...
*/
	...
	//CLog::Log(LOGDEBUG,"==========>>> cyk CDVDInputStreamFile::Read()  buf_size=%d", buf_size);
