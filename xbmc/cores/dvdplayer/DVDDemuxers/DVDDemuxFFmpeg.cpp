...
*/
	...
	if( m_pInput->IsStreamType(DVDSTREAM_TYPE_FFMPEG) ) /* 类型为DVDSTREAM_TYPE_FFMPEG */
	{
...
*/
	...
	CLog::Log(LOGDEBUG,"==========>>> cyk CDVDDemuxFFmpeg::Read()");
				...
				if (pPacket)
				{
					...
					if(pkt.dts != (int64_t)AV_NOPTS_VALUE)
					{
						...
						if(duration > stream->duration)
						{
							...
							if ((m_pFormatContext->duration == (int64_t)AV_NOPTS_VALUE && m_pFormatContext->file_size > 0) ||  (m_pFormatContext->duration != (int64_t)AV_NOPTS_VALUE && duration > m_pFormatContext->duration))
...
*/
	...
	for(unsigned i = 0; i < m_pFormatContext->nb_chapters; i++)
	{
		...
		if(m_iCurrentPts >= ConvertTimestamp(chapter->start, chapter->time_base.den, chapter->time_base.num) && m_iCurrentPts <  ConvertTimestamp(chapter->end,   chapter->time_base.den, chapter->time_base.num))
			return i + 1;
	}
#endif
	return 0;
...
*/
	...
	else
	{
		...
		// (Note that while the function was available earlier, the generic
		// metadata tags were not populated by default)
		AVMetadataTag *titleTag = m_dllAvFormat.av_metadata_get(m_pFormatContext->chapters[chapterIdx-1]->metadata,"title", NULL, 0);
		if (titleTag)
