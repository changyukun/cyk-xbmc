...
protected:
  ...
  CDVDDemux* m_pDemuxer; /* 	此值在函数OpenDemuxStream 中调用CDVDFactoryDemuxer::CreateDemuxer() 来
							  	创建的实例，所以指针m_pDemuxer 具体指向的是哪个类的实例
							  	要看CDVDFactoryDemuxer::CreateDemuxer() 中的代码调用，例如此指针
							  	指向CDVDDemuxFFmpeg 类的实例*/           // demuxer for current playing file
...
struct SPlayerState
{
  ...
  CPlayerOptions m_PlayerOptions; /* 调用OpenFile 的时候会对此进行赋值*/
};
