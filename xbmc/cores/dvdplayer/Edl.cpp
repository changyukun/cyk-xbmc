...
*/
	...
	*/
	...
	if (int(fFrameRate * 100) == 5994) // 59.940 fps = NTSC or 60i content
	{
		...
		CLog::Log(LOGDEBUG, "%s - Assuming NTSC or 60i interlaced content. Adjusted frames per second from %.3f (~59.940 fps) to %.3f",__FUNCTION__, fFrameRate, fFramesPerSecond);
	}
	else if (int(fFrameRate * 100) == 4795) // 47.952 fps = 24p -> NTSC conversion
	{
		...
		CLog::Log(LOGDEBUG, "%s - Assuming 24p -> NTSC conversion interlaced content. Adjusted frames per second from %.3f (~47.952 fps) to %.3f",__FUNCTION__, fFrameRate, fFramesPerSecond);
	}
	else if (iHeight == 576 && fFrameRate > 30.0) // PAL @ 50.0fps rather than PAL @ 25.0 fps. Can't use direct fps check of 50.0 as this is valid for 720p
	{
		...
		CLog::Log(LOGDEBUG, "%s - Assuming PAL interlaced content. Adjusted frames per second from %.3f (~50.00 fps) to %.3f",__FUNCTION__, fFrameRate, fFramesPerSecond);
	}
	else if (iHeight == 1080 && fFrameRate > 30.0) // Don't know of any 1080p content being broadcast at higher than 30.0 fps so assume 1080i
	{
		...
		CLog::Log(LOGDEBUG, "%s - Assuming 1080i interlaced content. Adjusted frames per second from %.3f to %.3f",__FUNCTION__, fFrameRate, fFramesPerSecond);
	}
	...
	*/
	else if (URIUtils::IsMythTV(strMovie) && !URIUtils::IsLiveTV(strMovie))
	{
		Clear(); // Don't clear in either ReadMyth* method as they are intended to be used together.
		CLog::Log(LOGDEBUG, "%s - Checking for commercial breaks within MythTV for: %s", __FUNCTION__,strMovie.c_str());
		bFound = ReadMythCommBreakList(strMovie, fFramesPerSecond);
		CLog::Log(LOGDEBUG, "%s - Checking for cut list within MythTV for: %s", __FUNCTION__, strMovie.c_str());
...
*/
	...
	while (edlFile.ReadString(strBuffer.GetBuffer(1024), 1024))
	{
		...
		int iAction;
		int iFieldsRead = sscanf(strBuffer, "%512s %512s %i", strFields[0].GetBuffer(512),strFields[1].GetBuffer(512), &iAction);
		...
		*/
		...
		switch (iAction)
		{
			case 0:
				...
				if (!AddCut(cut))
				{
					CLog::Log(LOGWARNING, "%s - Error adding cut from line %i in EDL file: %s", __FUNCTION__,iLine, edlFilename.c_str());
					continue;
				}
				break;
			case 1:
				...
				if (!AddCut(cut))
				{
					CLog::Log(LOGWARNING, "%s - Error adding mute from line %i in EDL file: %s", __FUNCTION__,iLine, edlFilename.c_str());
					continue;
				}
				break;
			case 2:
				...
				{
					CLog::Log(LOGWARNING, "%s - Error adding scene marker from line %i in EDL file: %s",__FUNCTION__, iLine, edlFilename.c_str());
					continue;
				}
				break;
			case 3:
				...
				if (!AddCut(cut))
				{
					CLog::Log(LOGWARNING, "%s - Error adding commercial break from line %i in EDL file: %s",__FUNCTION__, iLine, edlFilename.c_str());
					continue;
				}
				break;
			default:
				CLog::Log(LOGWARNING, "%s - Invalid action on line %i in EDL file: %s", __FUNCTION__, iLine,edlFilename.c_str());
				continue;
		}
	}
	...
	{
		CLog::Log(LOGDEBUG, "%s - Read %"PRIuS" cuts and %"PRIuS" scene markers in EDL file: %s", __FUNCTION__,m_vecCuts.size(), m_vecSceneMarkers.size(), edlFilename.c_str());
		return true;
	}
	else
	{
...
*/
	...
	{
		CLog::Log(LOGERROR, "%s - Invalid Comskip file: %s. Error reading line 1 - expected '%s' at start.",__FUNCTION__, comskipFilename.c_str(), COMSKIP_HEADER);
		comskipFile.Close();
		return false;
	}
	...
	if (sscanf(szBuffer, "FILE PROCESSING COMPLETE %i FRAMES AT %f", &iFrames, &fFrameRate) != 2)
	{
		...
		*/
		...
		CLog::Log(LOGWARNING, "%s - Frame rate not in Comskip file. Using detected frames per second: %.3f",__FUNCTION__, fFrameRate);
	}
	else
	...
	if (!bValid)
	{
		CLog::Log(LOGERROR, "%s - Invalid Comskip file: %s. Error on line %i. Clearing any valid commercial breaks found.",__FUNCTION__, comskipFilename.c_str(), iLine);
		Clear();
		return false;
	}
	else if (HasCut())
	{
		CLog::Log(LOGDEBUG, "%s - Read %"PRIuS" commercial breaks from Comskip file: %s", __FUNCTION__, m_vecCuts.size(),comskipFilename.c_str());
		return true;
	}
	else
	{
...
*/
	...
	*/
	...
	{
		CLog::Log(LOGERROR, "%s - Invalid VideoReDo file: %s. Error reading line 1 - expected %s. Only version 2 files are supported.",__FUNCTION__, videoReDoFilename.c_str(), VIDEOREDO_HEADER);
		videoReDoFile.Close();
		return false;
	}
	...
	if (!bValid)
	{
		CLog::Log(LOGERROR, "%s - Invalid VideoReDo file: %s. Error in line %i. Clearing any valid cuts or scenes found.",__FUNCTION__, videoReDoFilename.c_str(), iLine);
		Clear();
		return false;
	}
	else if (HasCut() || HasSceneMarker())
	{
		CLog::Log(LOGDEBUG, "%s - Read %"PRIuS" cuts and %"PRIuS" scene markers in VideoReDo file: %s", __FUNCTION__,m_vecCuts.size(), m_vecSceneMarkers.size(), videoReDoFilename.c_str());
		return true;
	}
	else
	{
		CLog::Log(LOGDEBUG, "%s - No cuts or scene markers found in VideoReDo file: %s", __FUNCTION__,videoReDoFilename.c_str());
		return false;
	}
}
...
*/
	...
	{
		CLog::Log(LOGERROR, "%s - Could not load Beyond TV file: %s. %s", __FUNCTION__, beyondTVFilename.c_str(),xmlDoc.ErrorDesc());
		return false;
	}

	if (xmlDoc.Error())
	{
		CLog::Log(LOGERROR, "%s - Could not parse Beyond TV file: %s. %s", __FUNCTION__, beyondTVFilename.c_str(),xmlDoc.ErrorDesc());
		return false;
	}
	...
	{
		CLog::Log(LOGERROR, "%s - Invalid Beyond TV file: %s. Expected root node to be <cutlist>", __FUNCTION__,beyondTVFilename.c_str());
		return false;
	}
	...
	if (!bValid)
	{
		CLog::Log(LOGERROR, "%s - Invalid Beyond TV file: %s. Clearing any valid commercial breaks found.", __FUNCTION__,beyondTVFilename.c_str());
		Clear();
		return false;
	}
	else if (HasCut())
	{
		CLog::Log(LOGDEBUG, "%s - Read %"PRIuS" commercial breaks from Beyond TV file: %s", __FUNCTION__, m_vecCuts.size(),beyondTVFilename.c_str());
		return true;
	}
	else
	{
		CLog::Log(LOGDEBUG, "%s - No commercial breaks found in Beyond TV file: %s", __FUNCTION__,beyondTVFilename.c_str());
		return false;
	}
}
...
*/
	...
	*/
	...
	{
		CLog::Log(LOGERROR, "%s - Error getting commercial break list from MythTV for: %s", __FUNCTION__,url.GetFileName().c_str());
		return false;
	}
	...
	if (HasCut())
	{
		CLog::Log(LOGDEBUG, "%s - Added %"PRIuS" commercial breaks from MythTV for: %s. Used detected frame rate of %.3f fps to calculate times from the frame markers.",__FUNCTION__, m_vecCuts.size(), url.GetFileName().c_str(), fFramesPerSecond);
		return true;
	}
	else
	{
...
*/
	...
	*/
	...
	{
		CLog::Log(LOGERROR, "%s - Error getting cut list from MythTV for: %s", __FUNCTION__,url.GetFileName().c_str());
		return false;
	}
