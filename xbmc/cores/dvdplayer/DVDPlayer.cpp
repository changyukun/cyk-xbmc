...
*/
	...
	{
		string filename = demuxer->GetFileName(); /*  获取文件名字*/
		int count = demuxer->GetNrOfStreams(); /*  获取文件中流的个数*/
		int source;
		...
		for(int i=0;i<count;i++)
		{
			CDemuxStream* stream = demuxer->GetStream(i);/* 获取文件中各个流的信息*/
...
CDVDPlayer::CDVDPlayer(IPlayerCallback& callback)
							...
							m_messenger("player"),
							m_dvdPlayerVideo(&m_clock, &m_overlayContainer, m_messenger), /* 实例了视频播放器*/
							m_dvdPlayerAudio(&m_clock, m_messenger),/* 实例了音频播放器*/
							m_dvdPlayerSubtitle(&m_overlayContainer),/* 实例了字幕播放器*/
							m_dvdPlayerTeletext(),
							m_ready(true)
{
/*
...
*/
	try
	{
		...
		/* 
			这里的Create 应该是调用CThread::Create 函数，因为类CDVDPlayer 继承了类CThread 
			这里就会继续创建一个线程，然后此线程就会执行process ，最后实质就会
			执行CDVDPlayer::Process()
		*/
		Create(); 
...
/*
	...
	说明:
		1、此函数在CThread::staticThread 中被调用，调用语句为pThread->OnStartup();
*/
	CLog::Log(LOGDEBUG,"==========>>> cyk OnStartup 1");
...
*/
	...
	if (filename.Find("dvd://") == 0 ||  filename.CompareNoCase("iso9660://video_ts/video_ts.ifo") == 0)
	{
...
retry:
	...
	/* OpenFile 方法调用的时候已经对m_filename 进行赋值了*/
	
	m_pInputStream = CDVDFactoryInputStream::CreateInputStream(this, m_filename, m_mimetype);/* 创建一个输入流实例*/
	...
	if(m_pInputStream == NULL)
	{
		...
		m_pInputStream->SetFileItem(m_item);/* 设定文件item */

	/*
		利用输入流的实例来打开文件( 此文件可能是本地文件，也可能是url  等)，并判断
		打开是否成功，不成功的还需要判断是否为蓝光
		注意m_pInputStream 所指向的实例是在CDVDFactoryInputStream::CreateInputStream() 中创建的
	*/
	...
	*/
	...
	if (!m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD) &&  !m_pInputStream->IsStreamType(DVDSTREAM_TYPE_TV) &&  !m_pInputStream->IsStreamType(DVDSTREAM_TYPE_HTSP))
	{
...
*/
			...
			*/
			...
			for(int i = 0; i < count; i++)
			{
				...
				if(s.source == STREAM_SOURCE_DEMUX) // Only demux streams
				{
					...
					if(s.channels > max_channels ||(s.channels == max_channels && codec_priority > max_codec_priority))
					{
	...
	for(int i = 0;i<count && !valid; i++)
	{
		...
		if ((s.source == STREAM_SOURCE_DEMUX_SUB || s.source == STREAM_SOURCE_TEXT) && OpenSubtitleStream(s.id, s.source))
			valid = true;
	}
...
*/
	...
	if (m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
	{
		...
		if(source_type  != STREAM_SOURCE_DEMUX|| stream->type != current.type|| stream->iId  == current.id)
			return false;
...
*/
	...
	if(m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))/* 如果输入流为DVD 需要特殊处理*/
	{
	...
	*/
	...
	while (!m_bAbortRequest)
	{
		...
		if((m_dvdPlayerAudio.m_messageQueue.GetDataSize() > 0 || m_CurrentAudio.id < 0) && (m_dvdPlayerVideo.m_messageQueue.GetDataSize() > 0 || m_CurrentVideo.id < 0))
			Sleep(0);
		...
		if (!pPacket)
		{
			...
			if(m_dvdPlayerAudio.m_messageQueue.GetDataSize() > 0 || m_dvdPlayerVideo.m_messageQueue.GetDataSize() > 0)
			{
				Sleep(100);
				continue;
			}
...
*/
	...
	bool drop = false;
	if (CheckPlayerInit(m_CurrentVideo, DVDPLAYER_VIDEO)) /* changyukun ---> */
		drop = true;
...
*/
	...
	if(caching == CACHESTATE_INIT)
	{
		...
		if((m_CurrentVideo.id < 0 || m_CurrentVideo.started) && (m_CurrentAudio.id < 0 || m_CurrentAudio.started))
			caching = CACHESTATE_PLAY;

		// handle situation that we get no data on one stream
		if(m_CurrentAudio.id >= 0 && m_CurrentVideo.id >= 0)
		{
			if ((!m_dvdPlayerAudio.AcceptsData() && !m_CurrentVideo.started)||  (!m_dvdPlayerVideo.AcceptsData() && !m_CurrentAudio.started))
			{
				caching = CACHESTATE_DONE;
	...
	if(caching == CACHESTATE_PLAY)
	{
		...
		if((m_CurrentVideo.id < 0 || !m_dvdPlayerVideo.IsStalled())&& (m_CurrentAudio.id < 0 || !m_dvdPlayerAudio.IsStalled()))
			caching = CACHESTATE_DONE;
	}
...
*/
	...
	if((current.type == STREAM_AUDIO && m_dvdPlayerAudio.IsStalled()) || (current.type == STREAM_VIDEO && m_dvdPlayerVideo.IsStalled()))
	{
		// don't start caching if it's only a single stream that has run dry
		if(m_dvdPlayerAudio.m_messageQueue.GetLevel() > 50 || m_dvdPlayerVideo.m_messageQueue.GetLevel() > 50)
			return false;

		if(m_pInputStream->IsStreamType(DVDSTREAM_TYPE_HTSP) || m_pInputStream->IsStreamType(DVDSTREAM_TYPE_TV))
...
*/
	...
	if(current.dts != DVD_NOPTS_VALUE && pPacket->dts < current.dts && current.inited)
	{
		...
		CLog::Log(LOGWARNING, "CDVDPlayer::CheckContinuity - wrapback of stream:%d, prev:%f, curr:%f, diff:%f" , current.type, current.dts, pPacket->dts, pPacket->dts - current.dts);
	}
...
*/
	...
	*/
	if(m_CurrentAudio.startpts != DVD_NOPTS_VALUE|| m_CurrentVideo.startpts != DVD_NOPTS_VALUE)
		return;

	if(m_CurrentAudio.dts == DVD_NOPTS_VALUE|| m_CurrentVideo.dts == DVD_NOPTS_VALUE)
		return;
	...
	if(cut.action == CEdl::CUT&& !(cut.end == m_EdlAutoSkipMarkers.cut || cut.start == m_EdlAutoSkipMarkers.cut)) // To prevent looping if same cut again
	{
...
*/
	...
	while (m_messenger.Get(&pMsg, 0) == MSGQ_OK)
	{
		try
		{
			if (pMsg->IsType(CDVDMsg::PLAYER_SEEK) && m_messenger.GetPacketCount(CDVDMsg::PLAYER_SEEK)== 0 &&  m_messenger.GetPacketCount(CDVDMsg::PLAYER_SEEK_CHAPTER) == 0)
			{
				/*
					seek 消息
				*/

				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> seek");
				...
				/* 
					调用demuxer 进行实际的定位，见函数中OpenDemuxStream 创建
					的m_pDemuxer，实质此处的seek 调用的是
				*/
			...
			else if (pMsg->IsType(CDVDMsg::PLAYER_SEEK_CHAPTER) && m_messenger.GetPacketCount(CDVDMsg::PLAYER_SEEK)== 0 && m_messenger.GetPacketCount(CDVDMsg::PLAYER_SEEK_CHAPTER) == 0)
			{
				/*
					chapter 消息
				*/

				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> chapter");
			...
			else if (pMsg->IsType(CDVDMsg::DEMUXER_RESET))
			{
				...
				*/
				...
				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> reset");
			...
			else if (pMsg->IsType(CDVDMsg::PLAYER_SET_AUDIOSTREAM))
			{
				...
				*/
				...
				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> SET_AUDIOSTREAM");
			...
			{
				/*
					SET_SUBTITLESTREAM 消息
				*/

				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> SET_SUBTITLESTREAM");
			...
			{
				/*
					SET_SUBTITLESTREAM_VISIBLE 消息
				*/

				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> SET_SUBTITLESTREAM_VISIBLE");
			...
			else if (pMsg->IsType(CDVDMsg::PLAYER_SET_STATE))
			{
				...
				*/
				...
				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> PLAYER_SET_STATE");
			...
			else if (pMsg->IsType(CDVDMsg::PLAYER_SET_RECORD))
			{
				...
				*/
				...
				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> PLAYER_SET_RECORD");
			...
			else if (pMsg->IsType(CDVDMsg::GENERAL_FLUSH))
			{
				...
				*/
				...
				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> GENERAL_FLUSH");
			...
			else if (pMsg->IsType(CDVDMsg::PLAYER_SETSPEED))
			{
				...
				*/
				...
				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> PLAYER_SETSPEED");
								...
								(pMsg->IsType(CDVDMsg::PLAYER_CHANNEL_SELECT) && m_messenger.GetPacketCount(CDVDMsg::PLAYER_CHANNEL_SELECT) == 0))
			{
				...
				*/
				...
				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> xxxx");
			...
			{
				/*
					设定GENERAL_GUI_ACTION 消息
				*/

				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> GENERAL_GUI_ACTION");
			...
			else if (pMsg->IsType(CDVDMsg::PLAYER_STARTED))
			{
				...
				*/
				...
				CLog::Log(LOGDEBUG,"==========>>> cyk CDVDPlayer::HandleMessages() ---> PLAYER_STARTED");
...
*/
	...
	if(((bPlus && GetChapter() < GetChapterCount())|| (!bPlus && GetChapter() > 1)) && bLargeStep)
	{
		if(bPlus)
	...
	if (m_Edl.HasCut())
	{
		...
		*/
		else if (bPlus && bLargeStep && clock >= m_EdlAutoSkipMarkers.commbreak_start && clock <= m_EdlAutoSkipMarkers.commbreak_end)
		{
	...
	if(g_application.CurrentFileItem().IsStack() && (seek > GetTotalTimeInMsec() || seek < 0))
	{
...
*/
	...
	strAudioInfo.Format("D(%s) P(%s)", m_State.demux_audio.c_str() , m_dvdPlayerAudio.GetPlayerInfo().c_str());
}
...
*/
	CSingleLock lock(m_StateSection);
	strVideoInfo.Format("D(%s) P(%s)", m_State.demux_video.c_str() , m_dvdPlayerVideo.GetPlayerInfo().c_str());
}
...
*/
	if (!m_bStop)
	{
		...
		{
			strBuf.AppendFormat(" cache:%s %2.0f%%", StringUtils::SizeToString(m_State.cache_bytes).c_str(), m_State.cache_level * 100);
...
*/
	...
	{
		if (!m_dvdPlayerAudio.OpenStream( hint )) /* changyukun BBB--2--BBB  间接创建启动音频解码器线程*/
		{
