...
*/
	...
	{
		if( (pCodec = OpenCodec(new CDVDVideoCodecLibMpeg2(), hint, options)) ) 
			return pCodec;
	}
	...
	if (!hint.software && g_guiSettings.GetBool("videoplayer.usevda"))
	{
		if (g_sysinfo.HasVDADecoder())
		{
			if (hint.codec == CODEC_ID_H264 && !hint.ptsinvalid)
			{
				...
				if ( (pCodec = OpenCodec(new CDVDVideoCodecVDA(), hint, options)) ) 
					return pCodec;
			}
		}
	}
		...
		if (g_sysinfo.HasVideoToolBoxDecoder())
		{
			switch(hint.codec)
			{
				...
				CLog::Log(LOGINFO, "Apple VideoToolBox Decoder...");
					if((pCodec = OpenCodec(new CDVDVideoCodecVideoToolBox(), hint, options))) 
						return pCodec;
					break;
		...
		if (CCrystalHD::GetInstance()->DevicePresent())
		{
			switch(hint.codec)
			{
				...
				case CODEC_ID_MPEG2VIDEO:
					...
					if ( (pCodec = OpenCodec(new CDVDVideoCodecCrystalHD(), hint, options)) ) 
						return pCodec;
					break;
	...
	if (g_guiSettings.GetBool("videoplayer.useomx") && !hint.software )
	{
		if (hint.codec == CODEC_ID_H264 || hint.codec == CODEC_ID_MPEG2VIDEO || hint.codec == CODEC_ID_VC1)
		{
			...
			if((pCodec = OpenCodec(new CDVDVideoCodecOpenMax(), hint, options)) ) 
				return pCodec;
		}
	}
#endif
