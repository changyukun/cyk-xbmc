...
CDecoder::CDecoder() : m_event(true)
{
/*
	参数:
		1、
...
*/
	...
	if(AIdentifier.VendorId == PCIV_nVidia && !CDVDCodecUtils::IsVP3CompatibleWidth(avctx->coded_width))
	{
...
*/
	...
	if(m_format.SampleWidth  == 0|| m_format.SampleHeight == 0)
	{
	...
	if(avctx->codec_id != CODEC_ID_H264 && avctx->codec_id != CODEC_ID_VC1 && avctx->codec_id != CODEC_ID_WMV3)
		return 0;
...
*/
	CSingleLock lock(m_section);
	if(avctx->coded_width  != m_format.SampleWidth || avctx->coded_height != m_format.SampleHeight)
	{
		Close();
		if(!Open(avctx, avctx->pix_fmt, m_shared))
		{
...
*/
	...
	{
		if(NoDeintProcForProgDevices[idx].VendorID == AIdentifier.VendorId && NoDeintProcForProgDevices[idx].DeviceID == AIdentifier.DeviceId)
		{
...
*/
	...
	m_progressive = m_deinterlace_mode == VS_DEINTERLACEMODE_OFF || (   m_interlace_method != VS_INTERLACEMETHOD_DXVA_BOB && m_interlace_method != VS_INTERLACEMETHOD_DXVA_BEST);
...
*/
	...
	if(m_interlace_method != method || m_deinterlace_mode != mode || !m_process)
	{
	...
	blt.ProcAmpValues.Brightness = ConvertRange( m_brightness, g_settings.m_currentVideoSettings.m_Brightness  , 0, 100, 50);
	blt.ProcAmpValues.Contrast   = ConvertRange( m_contrast, g_settings.m_currentVideoSettings.m_Contrast  , 0, 100, 50);
