...
/*
	参数:
		1、strCore 	: 传入一个名字
		2、callback 	: 传入一个IPlayerCallback 类的实例，通常就是CApplication  的实例
	...
	说明:
		1、
*/
  	return CreatePlayer( GetPlayerCore(strCore), callback ); /* 见函数GetPlayerCore 的说明*/
}
...
/*
	...
	说明:
		1、
*/
	if (!s_vecCoreConfigs.size() || eCore-1 > s_vecCoreConfigs.size()-1)/* 判断此id 是否为"  配置实例数组" 中的有效id */
		return NULL;
...
/*
	参数:
		1、strCoreName : 传入一个播放器的名字
		
	返回:
		1、
		
	说明:
		1、首先判断传入的播放器的名字是否为默认的，如果是取出默认的名字
			不是默认的就用传入的播放器的名字

			根据获得到的名字在"  配置实例数组" 中查找到相应的配置单元，并返回此配置
			单元的id ( 从1 开始的)
*/
	if (!strCoreName.empty())
	{
		...
		if (strCoreName.Equals("audiodefaultplayer", false)) 
			strRealCoreName = g_advancedSettings.m_audioDefaultPlayer;
		else if (strCoreName.Equals("videodefaultplayer", false))
			strRealCoreName = g_advancedSettings.m_videoDefaultPlayer;
		else if (strCoreName.Equals("videodefaultdvdplayer", false)) 
			strRealCoreName = g_advancedSettings.m_videoDefaultDVDPlayer;
		else 
...
/*
	...
	说明:
		1、实质就是返回"  配置实例数组" 中对应此id 的那个实例的名字
*/
...
/*
	...
	说明:
		1、实质就是返回"  配置实例数组" 中对应此名字 的那个实例
*/
	PLAYERCOREID id = GetPlayerCore(strCoreName);
	if (id != EPC_NONE) 
		return s_vecCoreConfigs[id-1];
	else
		return NULL;
}
...
/*
	参数:
		1、vecCores : 传入一个容器( 用于返回音频播放器、视频播放器id 的数组)
		
	返回:
		1、
		
	说明:
		1、实质就是将"  配置实例数组" 中所有为音频或者视频播放器的id 返回到容器中
*/
...
/*
	参数:
		1、strCore 	: 传入一个容器( 用于返回音频播放器、视频播放器id 的数组)
		2、audio 	: 传入一个参数，是否为音频
		3、video 	: 传入一个参数，是否为视频
		
	返回:
		1、
		
	说明:
		1、实质就是将"  配置实例数组" 中所有满足参数2 和参数3 的播放器的id 返回到容器中
*/
...
*/
	...
	if( PAPlayer::HandlesType(url.GetFileType()) )
	{
		...
		if (bAdd)
		{
			...
			else if (url.GetFileType().Equals("ac3") || url.GetFileType().Equals("dts"))
			{
...
/*
	参数:
		1、pConfig 	: 传入一个Xml 类对象，理解为tinyxml 分析xml 得到的结果实例
		2、clear 	: 传入一个参数，是否需要清除"  配置实例数组"  和"  选择规则数组"  中的所有元素
...
*/
	...
	if (pPlayers)
	{
		...
		while (pPlayer)
		{
			...
			if (type == "dvdplayer" || type == "mplayer")
				eCore = EPC_DVDPLAYER;
			
			if (type == "paplayer" ) 
				eCore = EPC_PAPLAYER;
			
			if (type == "externalplayer" ) 
				eCore = EPC_EXTPLAYER;
