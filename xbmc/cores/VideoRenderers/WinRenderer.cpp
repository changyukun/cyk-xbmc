...
/*
	...
	说明:
		1、
*/
	if(m_sourceWidth  != width|| m_sourceHeight != height)
	{
		m_sourceWidth       = width;
...
*/
	if (m_renderMethod == RENDER_DXVA)
	{
		...
		buf->id = m_processor.Add(picture); /* 调用处理器的添加图片的方法*/
		return true;
	}
	return false;
}
...
/*
	...
	说明:
		1、
*/
	if(mode == VS_DEINTERLACEMODE_OFF|| mode == VS_DEINTERLACEMODE_AUTO|| mode == VS_DEINTERLACEMODE_FORCE)
		return true;
...
*/
	...
	if (m_renderMethod == RENDER_DXVA)
	{
		if(method == VS_INTERLACEMETHOD_DXVA_BOB|| method == VS_INTERLACEMETHOD_DXVA_BEST)
			return true;
	}

	if(!m_dxvaDecoding && (   method == VS_INTERLACEMETHOD_DEINTERLACE || method == VS_INTERLACEMETHOD_DEINTERLACE_HALF || method == VS_INTERLACEMETHOD_SW_BLEND))
		return true;
...
*/
	...
	{
		if(m_deviceCaps.PixelShaderVersion >= D3DPS_VERSION(2, 0)&& (   method == VS_SCALINGMETHOD_AUTO|| method == VS_SCALINGMETHOD_LINEAR))
			return true;
	...
	else if(m_renderMethod == RENDER_SW)
	{
		if(method == VS_SCALINGMETHOD_AUTO|| method == VS_SCALINGMETHOD_NEAREST)
			return true;
		if(method == VS_SCALINGMETHOD_LINEAR && m_deviceCaps.TextureFilterCaps & D3DPTFILTERCAPS_MINFLINEAR && m_deviceCaps.TextureFilterCaps & D3DPTFILTERCAPS_MAGFLINEAR)
			return true;
	}
	return false;
}
...
*/
	for(unsigned i = 0; i < m_activeplanes; i++)
	{
		if(planes[i].texture.Get()&& planes[i].texture.LockRect(0, &planes[i].rect, NULL, D3DLOCK_DISCARD) == false)
		{
			memset(&planes[i].rect, 0, sizeof(planes[i].rect));
