...
*/
	...
	for (int i = 0; i < (int)g_advancedSettings.m_videoAdjustRefreshOverrides.size(); i++)
	{
		...
		for (size_t j = (int)RES_DESKTOP; j < g_settings.m_ResInfo.size(); j++)
		{
			...
			{
				if (g_settings.m_ResInfo[j].fRefreshRate <= override.refreshmax && g_settings.m_ResInfo[j].fRefreshRate >= override.refreshmin)
				{
					m_resolution = (RESOLUTION)j;

					if (fallback)
					{
						CLog::Log(LOGDEBUG, "Found Resolution %s (%d) from fallback (refreshmin:%.3f refreshmax:%.3f)",g_settings.m_ResInfo[m_resolution].strMode.c_str(), m_resolution, override.refreshmin, override.refreshmax);
					}
					else
					{
...
*/
	...
	for (size_t i = (int)RES_DESKTOP; i < g_settings.m_ResInfo.size(); i++)
	{
		...
		if (info.iWidth  != curr.iWidth ||  info.iHeight != curr.iHeight ||  info.iScreen != curr.iScreen ||  info.fRefreshRate < (fps * multiplier / 1.001) - 0.001)
			continue;
		...
		if ((i_weight <  c_weight) ||  (i_weight == c_weight && info.fRefreshRate > curr.fRefreshRate))
...
*/
	...
	bool is43 = (sourceFrameRatio < 8.f/(3.f*sqrt(3.f)) && g_settings.m_currentVideoSettings.m_ViewMode == VIEW_MODE_NORMAL);

	g_settings.m_fVerticalShift = 0.0f;
	g_settings.m_bNonLinStretch = false;

	if ( g_settings.m_currentVideoSettings.m_ViewMode == VIEW_MODE_ZOOM || (is43 && g_guiSettings.GetInt("videoplayer.stretch43") == VIEW_MODE_ZOOM))
	...
	else if ( g_settings.m_currentVideoSettings.m_ViewMode == VIEW_MODE_WIDE_ZOOM || (is43 && g_guiSettings.GetInt("videoplayer.stretch43") == VIEW_MODE_WIDE_ZOOM))
	{ // super zoom
	...
	else if ( g_settings.m_currentVideoSettings.m_ViewMode == VIEW_MODE_STRETCH_16x9 || (is43 && g_guiSettings.GetInt("videoplayer.stretch43") == VIEW_MODE_STRETCH_16x9))
