...
protected:
	...
	int m_iPlayableItems; /* 相当于记录容器m_vecItems  内可播放单元的个数，见add  方法会对其加1  操作，而SetUnPlayable 方法减1 操作*/
	bool m_bShuffled;
	bool m_bWasPlayed;

	//  CFileItemList m_vecItems;
	std::vector <CFileItemPtr> m_vecItems; /* FileItem 指针的容器，即容器里面存放的都是指向FileItem  的指针*/
