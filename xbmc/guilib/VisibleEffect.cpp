...
*/
	...
	if (TIXML_SUCCESS == node->QueryFloatAttribute("time", &temp)) /* 从xml  里面解析出来长度的值*/
		m_length = (unsigned int)(temp * g_SkinInfo->GetEffectsSlowdown());
	
	if (TIXML_SUCCESS == node->QueryFloatAttribute("delay", &temp))   /* 从xml  里面解析出来延迟的值*/
...
/*
	...
	说明:
		1、根据xml 节点的信息构造一个不同的m_pTweener  内容
*/
...
*/
	...
	for (unsigned int i = 0; i < src.m_effects.size(); i++)
	{
		...
		else if (src.m_effects[i]->GetType() == CAnimEffect::EFFECT_TYPE_ROTATE_X || src.m_effects[i]->GetType() == CAnimEffect::EFFECT_TYPE_ROTATE_Y ||src.m_effects[i]->GetType() == CAnimEffect::EFFECT_TYPE_ROTATE_Z)
