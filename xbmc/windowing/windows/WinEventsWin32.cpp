...
/*
	...
	说明:
		1、cyk @@@@@@@@@@@@@@@@ 窗口事件回调函数，如xbmc  主窗口激活时，就会响应WM_ACTIVATE 消息
*/
...
*/
	...
	switch (gi.dwID)
	{
		...
		case GID_PAN:
			{
				g_application.OnAction(CAction(ACTION_GESTURE_PAN, 0, (float)point.x, (float)point.y,(float)(point.x - m_lastGesturePosX), (float)(point.y - m_lastGesturePosY)));
				m_lastGesturePosX = point.x;
				m_lastGesturePosY = point.y;
			}
			break;
