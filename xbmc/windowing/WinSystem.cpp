...
/*
	...
	说明:
		1、
*/
	return ( i.width < j.width || (i.width == j.width && i.height < j.height));
}
...
*/
	...
	for (unsigned int idx = 0; idx < refreshrates.size(); idx++)
		if (   refreshrates[idx].RefreshRate == RefreshRate&& refreshrates[idx].Interlaced  == Interlaced )
			return; // already taken care of.
...
/*
	...
	说明:
		1、
*/
	return (   (i.RefreshRate < j.RefreshRate) || (i.RefreshRate == j.RefreshRate && !i.Interlaced));
}
...
*/
	...
	for (unsigned int idx = RES_DESKTOP; idx < g_settings.m_ResInfo.size(); idx++)
		if (g_settings.m_ResInfo[idx].iScreen == screen && g_settings.m_ResInfo[idx].iWidth  == width && g_settings.m_ResInfo[idx].iHeight == height)
