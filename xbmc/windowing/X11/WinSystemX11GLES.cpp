...
CWinSystemX11GLES::CWinSystemX11GLES() : CWinSystemBase() , m_eglOMXContext(0)
{
/*
	参数:
		1、
...
*/
	...
	EGLint maj, min;
	if ((m_dpy = XOpenDisplay(NULL)) && (m_eglDisplay = eglGetDisplay((EGLNativeDisplayType)m_dpy)) && (val = eglInitialize(m_eglDisplay, &maj, &min)))
	{
		SDL_EnableUNICODE(1);
