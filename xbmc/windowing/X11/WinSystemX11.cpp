...
*/
	...
	if (fullScreen && (res.iWidth != desktop.iWidth || res.iHeight != desktop.iHeight || res.fRefreshRate != desktop.fRefreshRate || res.iScreen != desktop.iScreen))
	{
...
*/
	...
	if (XGetWindowAttributes(m_dpy, m_glWindow, &winAttr))
	{
		...
		{
			CLog::Log(LOGWARNING, "Visual 0x%x of the SDL window is not suitable, looking for another one...", (unsigned) vInfo->visualid);
...
*/
	...
	CLog::Log(LOGERROR, "CWinSystemX11::XErrorHandler: %s, type:%i, serial:%lu, error_code:%i, request_code:%i minor_code:%i", buf, error->type, error->serial, (int)error->error_code, (int)error->request_code, (int)error->minor_code);
