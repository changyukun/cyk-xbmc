...
/*
	参数:
		1、strSettingsFile 	: 传入一个设置的Xml 文件路径
...
CLog::Log(LOGDEBUG,"==========>>> cyk LoadSettings 1");
  ...
  CLog::Log(LOGDEBUG,"==========>>> cyk LoadSettings 2");
...
/*
	...
	说明:
		1、首先调用tinyxml  模块对传入的xml 文件进行分析，然后得到xml 的一个实体，
			然后再利用这个实体进行调用 CPlayerCoreFactory::LoadConfiguration 处理
*/
