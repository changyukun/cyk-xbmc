...
*/
	...
	* can now be caught using c++ try catch */
	...
	CLog::Log(LOGDEBUG,"==========>>> cyk CApplication::Create 1");
...
*/
	...
	if (promptuser)
	{
		if (server->GetNumberOfClients() > 0)
		{
			...
			if (!CGUIDialogYesNo::ShowAndGetInput(13140, 13141, 13142, 20022,-1, -1, cancelled, 10000)|| cancelled)
			{
...
*/
	...
	{ // we can't register visible condition in dialog's ctor because infomanager is cleared when unloading skin
		...
		if (overlay) 
			overlay->SetVisibleCondition("skin.hasmusicoverlay");
	}
...
*/
	...
	/*
		1、CApplication::RenderNoPresent()
		2、g_windowManager.Render()   --->  CGUIWindowManager::Render()
		3、RenderPass()	 --->  CGUIWindowManager::RenderPass()
		4、DoRender()  --->   CGUIWindow::DoRender()
		5、CGUIControlGroup::DoRender()
	*/
	if (RenderNoPresent())
		hasRendered = true;

	/* ===================>  Direct3D 第2  步========>  EndScene() */
...
/*
	...
	说明:
		1、此函数会被多次调用，即使一个电影是有多个分段组成的，则每播放一个分段之后就会调用
			一次此函数进行下一个分段的播放的
...
*/
	...
	if (!m_pPlayer)
	{
		CLog::Log(LOGDEBUG,"==========>>> cyk PlayFile 2");
	...
	if (m_pPlayer)
	{
		...
		CLog::Log(LOGDEBUG,"==========>>> cyk PlayFile 3");
	...
	if(bResult)
	{
		...
		if( IsPlayingVideo() )
		{
			...
			if( options.fullscreen && g_renderManager.IsStarted()&& g_windowManager.GetActiveWindow() != WINDOW_FULLSCREEN_VIDEO )
				SwitchToFullScreen();
...
*/
	...
	if ( IsPlaying() )
	{
		...
		if (iWin == WINDOW_VISUALISATION||  iWin == WINDOW_FULLSCREEN_VIDEO)
...
*/
	...
	bool maybeScreensaver = !m_dpmsIsActive && !m_bScreenSave && !g_guiSettings.GetString("screensaver.mode").IsEmpty();
	bool maybeDPMS = !m_dpmsIsActive && m_dpms->IsSupported() && g_guiSettings.GetInt("powermanagement.displaysoff") > 0;
	...
	if (maybeDPMS&& elapsed > g_guiSettings.GetInt("powermanagement.displaysoff") * 60)
	{
		ToggleDPMS(false);
		WakeUpScreenSaver();
	}
	else if (maybeScreensaver&& elapsed > g_guiSettings.GetInt("screensaver.time") * 60)
	{
		ActivateScreenSaver();
	}
}
...
*/
	...
	g_windowManager.DispatchThreadMessages(); /* 先分发处理窗口管理相关容器中的消息，见此方法的说明，可能会调用到相关窗口的OnMessage  方法*/
	...
	m_applicationMessenger.ProcessMessages();/* 见函数内部分析*/
...
*/
	...
	if (m_pPlayer->IsPaused())
	{
		if (((m_iPlaySpeed > 1) && (iSpeed > m_iPlaySpeed)) ||((m_iPlaySpeed < -1) && (iSpeed < m_iPlaySpeed)))
		{
...
/*
	...
	说明:
		1、
*/
	CLog::Log(LOGDEBUG,"==========>>> cyk @@@@@@@@@@@@@@@@@@@@@@@ SeekTime ");
