...
*/
	...
	else if (PlayList_Check(pObject))
	{
		...
		g_playlistPlayer.SetCurrentPlaylist(pPlayList->iPlayList); /* 将播放列表设定到全局的播放列表实例中*/

		CPyThreadState pyState;
		g_application.getApplicationMessenger().PlayListPlayerPlay(); /* cyk  ---- 从python 脚本中获取到播放列表了，然后进行播放*/
	}
