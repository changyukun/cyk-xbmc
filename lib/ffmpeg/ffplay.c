...
*/
	...
	if (!vp->bmp ||
#if CONFIG_AVFILTER
	vp->width  != is->out_video_filter->inputs[0]->w ||vp->height != is->out_video_filter->inputs[0]->h) 
	{
#else
	vp->width != is->video_st->codec->width || vp->height != is->video_st->codec->height) 
	{
#endif
		SDL_Event event;
   	...
   	if (vp->bmp) 
	{
		...
		av_picture_copy(&pict, &pict_src, vp->pix_fmt, vp->width, vp->height);
#else
...
*/
	...
	if(pic->reference) 
		perms |= AV_PERM_READ | AV_PERM_PRESERVE;
...
*/
	...
	if ((codec->width != ref->video->w) || (codec->height != ref->video->h) ||(codec->pix_fmt != ref->format)) 
	{
...
*/
	...
	if (((is->av_sync_type == AV_SYNC_VIDEO_MASTER && is->video_st) || is->av_sync_type == AV_SYNC_EXTERNAL_CLOCK)) 
	{
...
*/
	...
	printf("\n");
	av_opt_show2(avcodec_opts[0], NULL,AV_OPT_FLAG_DECODING_PARAM, 0);
	printf("\n");
	av_opt_show2(avformat_opts, NULL,AV_OPT_FLAG_DECODING_PARAM, 0);
#if !CONFIG_AVFILTER
	printf("\n");
	av_opt_show2(sws_opts, NULL,AV_OPT_FLAG_ENCODING_PARAM, 0);
#endif
	printf("\nWhile playing:\n"
				"q, ESC              quit\n"
