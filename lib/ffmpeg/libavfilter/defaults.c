...
*/
	...
	picref = avfilter_get_video_buffer_ref_from_arrays(data, linesize, perms, w, h, link->format);
	if (!picref)
	{
		av_free(data[0]);
		return NULL;
...
*/
	...
	if (outlink) 
	{
		...
		avfilter_start_frame(outlink, avfilter_ref_buffer(outlink->out_buf, ~0)); /* 调用avfilter_start_frame 进行下一级滤镜的处理*/
	}
}
...
*/
	...
	if (outlink) 
	{
		outlink->out_buf = avfilter_default_get_audio_buffer(	inlink, 
															AV_PERM_WRITE, 
															samplesref->format,
...
/*
	...
	说明:
		1、
*/
    	return avfilter_get_audio_buffer(link->dst->outputs[0], perms, sample_fmt,size, channel_layout, packed);
}
